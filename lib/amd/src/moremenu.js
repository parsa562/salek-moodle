...
export default menu => {
    ...
    // Recalculate the menu against its currently available space.
    const recalculate = () => {
        autoCollapse(menu);
        menu_navigation(menu);
    };

    // Populate the more dropdown menu with additional nodes if necessary.
    recalculate();

    // The available width can change without a window resize, for example when
    // a drawer opens or closes. Observe the menu container so MoreMenu stays
    // synchronized with its actual available width.
    if ('ResizeObserver' in window) {
        let lastObservedWidth = menu.parentNode.clientWidth;
        ...
        const resizeTarget = menu.closest('.navbar') || menu.parentNode;
        resizeObserver.observe(resizeTarget);
    }

    // Keep the existing behaviour for real viewport resizes.
